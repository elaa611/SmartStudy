...
import 'quiz_play_screen.dart'; // écran où l'utilisateur passe le quiz une fois généré

/// Modèle simple pour représenter une matière (identique à celui de study_screen.dart)
class Matiere {
  final int id;
  final String nom;
  final IconData icon;
...
class GenerateQuizScreen extends StatefulWidget {
  /// Optionnel : permet de pré-sélectionner une matière + des cours,
  /// par exemple depuis SummaryResultScreen ("Generate Quiz from this").
...
class _GenerateQuizScreenState extends State<GenerateQuizScreen> {
  ...
  // ============================================================
  // ÉTAPE 1 : charger les matières de l'utilisateur (comme dans study_screen.dart)
  // ============================================================
  Future<void> _chargerMatieres() async {
    ...
    // Pré-sélection éventuelle (venant de SummaryResultScreen par exemple)
  ...
  // ============================================================
  // ÉTAPE 3 : appeler l'Edge Function Supabase pour générer le quiz
  // ============================================================
  Future<void> _genererQuiz() async {
    ...
    try {
      // supabase.functions.invoke appelle directement l'Edge Function
      // en passant automatiquement le JWT de l'utilisateur connecté
      // (donc pas besoin d'envoyer user_id à la main, l'Edge Function
      // le récupère depuis le token).
      ...
      // On envoie l'utilisateur directement sur l'écran de jeu du quiz généré
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => QuizPlayScreen(quizId: quizId),
        ),
      );
    } catch (e) {
      _showError('Impossible de générer le quiz : $e');
    } finally {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'The quiz will be generated only from the selected courses.',
  ...
  Widget _buildDifficulte() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Difficulty', style: TextStyle(fontSize: 17,fontWeight:  FontWeight.bold)),
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Generate a New Quiz',
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: _generationEnCours ? null : _genererQuiz,
                  ...
                  label: Text(
                    _generationEnCours ? 'Generation in progress...' : 'Generate Quiz',
